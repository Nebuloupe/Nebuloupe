...
def run_check(session, tf_path="."):
    ...
    inbound SSH (port 22) from 0.0.0.0/0 or ::/0 (unrestricted internet access).
    """
    findings = []
    ...
    if not tf_files:
        findings.append(create_finding(
            "IAC-AWS-SG-01", "Terraform SG Open SSH", "Info",
            "PASS", "N/A",
    ...
    # Find all security group resources (both aws_security_group and aws_security_group_rule)
    ...
    for sg_name, sg_body in sg_pattern.findall(all_content):
        ...
        if open_ssh:
            findings.append(create_finding(
                "IAC-AWS-SG-01", "Terraform SG Open SSH", "Critical",
                "FAIL", f"aws_security_group.{sg_name}",
                f"Security group '{sg_name}' allows inbound SSH (port 22) from 0.0.0.0/0. "
                "This exposes SSH to the entire internet, enabling brute-force and credential "
                "stuffing attacks.",
                "Restrict SSH access to specific trusted CIDR ranges or use AWS Systems Manager "
                "Session Manager for shell access instead.",
                {"security_group": sg_name, "port": 22, "open_cidr": "0.0.0.0/0"}
            ))
        else:
            findings.append(create_finding(
                "IAC-AWS-SG-01", "Terraform SG Open SSH", "Low",
                "PASS", f"aws_security_group.{sg_name}",
                f"Security group '{sg_name}' does not allow unrestricted SSH access from the internet.",
                "No action required.",
                {"security_group": sg_name, "ssh_open": False}
            ))
    ...
    for rule_name, rule_body in sg_rule_pattern.findall(all_content):
        ...
        if from_port and to_port:
            ...
            if fp <= 22 <= tp:
                ...
                if '0.0.0.0/0' in all_cidrs or '::/0' in all_cidrs:
                    findings.append(create_finding(
                        "IAC-AWS-SG-01", "Terraform SG Rule Open SSH", "Critical",
                        "FAIL", f"aws_security_group_rule.{rule_name}",
                        f"Security group rule '{rule_name}' allows inbound SSH (port 22) from 0.0.0.0/0.",
                        "Restrict the CIDR range to trusted IP ranges only.",
                        {"rule": rule_name, "port": 22, "open_cidr": "0.0.0.0/0"}
                    ))
