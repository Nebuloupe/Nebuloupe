...
# Keywords that identify a Cloud SQL changes metric filter
SQL_KEYWORDS = ['cloudsql.instances', 'v1beta4.instances', 'sqladmin', 'sql_instance']
...
def run_check(project_id: str):
    ...
    # Step 1: look for a log-based metric covering Cloud SQL changes
    ...
    for m in metrics:
        filt = m.get('filter', '')
        if any(kw in filt for kw in SQL_KEYWORDS):
            sql_metric = m
            break

    metric_exists = sql_metric is not None
    ...
    if metric_exists:
        ...
        for policy in alerts_resp.get('alertPolicies', []):
            for condition in policy.get('conditions', []):
                ...
                if sql_metric['name'].split('/')[-1] in filter_str:
                    alert_exists = True
                    break
    ...
    findings.append(create_finding(
        "GCP-LOG-08",
        "Cloud SQL Instance Changes Alert Configured",
        "Medium" if not fully_configured else "Low",
        "FAIL" if not fully_configured else "PASS",
        f"projects/{project_id}",
        "No log-based metric and alert found for Cloud SQL instance configuration changes." if not fully_configured
        else "Log-based metric and alert for Cloud SQL instance changes are configured.",
        "Create a log-based metric targeting Cloud SQL instance mutations and attach a Cloud Monitoring alert." if not fully_configured
        else "No action required.",
        {
            "metric_exists": metric_exists,
            "alert_exists": alert_exists,
            "metric_name": sql_metric['name'] if sql_metric else None
        }
    ))
