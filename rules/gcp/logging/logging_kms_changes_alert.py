...
# Keywords that identify a Cloud KMS changes metric filter
KMS_KEYWORDS = ['cloudkms.googleapis.com', 'KeyRing', 'CryptoKey',
                'google.cloud.kms', 'kms_key', 'cryptoKeyVersion']
...
def run_check(project_id: str):
    ...
    for m in metrics:
        filt = m.get('filter', '')
        if any(kw in filt for kw in KMS_KEYWORDS):
            kms_metric = m
            break

    metric_exists = kms_metric is not None
    ...
    if metric_exists:
        ...
        for policy in alerts_resp.get('alertPolicies', []):
            for condition in policy.get('conditions', []):
                ...
                if kms_metric['name'].split('/')[-1] in filter_str:
                    alert_exists = True
                    break
    ...
    findings.append(create_finding(
        "GCP-LOG-10",
        "KMS Key Changes Alert Configured",
        "High" if not fully_configured else "Low",
        "FAIL" if not fully_configured else "PASS",
        f"projects/{project_id}",
        "No log-based metric and alert found for Cloud KMS key or key ring changes." if not fully_configured
        else "Log-based metric and alert for KMS key changes are configured.",
        "Create a log-based metric targeting KMS key/key ring mutations and attach a Cloud Monitoring alerting policy." if not fully_configured
        else "No action required.",
        {
            "metric_exists": metric_exists,
            "alert_exists": alert_exists,
            "metric_name": kms_metric['name'] if kms_metric else None
        }
    ))
