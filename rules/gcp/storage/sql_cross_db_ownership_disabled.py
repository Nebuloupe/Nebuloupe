...
TARGET_FLAG = "cross db ownership chaining"
SAFE_VALUE = "off"
...
def run_check(project_id: str):
    ...
    try:
        ...
        return [create_finding(
            "GCP-SQL-04", "Cloud SQL Cross-DB Ownership Chaining Disabled", "High", "ERROR",
    ...
    if not sql_server_instances:
        return [create_finding(
            "GCP-SQL-04", "Cloud SQL Cross-DB Ownership Chaining Disabled", "Low", "PASS",
            project_id, f"projects/{project_id}",
    ...
    for instance in sql_server_instances:
        ...
        # If flag is absent, SQL Server defaults to OFF (safe), but explicit is better
        ...
        desc = (
            ...
            + ("explicitly set to 'off'." if flag_value == SAFE_VALUE else
               "not explicitly set (defaults to off — recommend explicit configuration)."
               if flag_value is None else
               f"set to '{flag_value}', enabling cross-database ownership chaining.")
        )

        findings.append(create_finding(
            "GCP-SQL-04", "Cloud SQL Cross-DB Ownership Chaining Disabled",
            ...
            "instances to prevent privilege escalation via cross-database ownership chains.",
            {
                "flag": TARGET_FLAG,
