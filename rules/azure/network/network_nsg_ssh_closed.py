...
def is_port_exposed(ports_property, target_ports=(22,)):
    """
...
def run_check(credential):
    """
    Checks if Azure Network Security Groups have rules exposing Port 22 (SSH) to the world.
    """
    findings = []
    ...
    try:
                ...
                try:
                    for rule in nsg.security_rules:
                        ...
                        if rule.direction.lower() == 'inbound' and rule.access.lower() == 'allow':
                            ...
                            if is_source_public(source_prefixes) and is_port_exposed(dest_ports, [22]):
                                
                                findings.append({
                                    "finding_id": f"NL-AZ-{uuid.uuid4().hex[:6].upper()}",
                                    "rule_id": "CIS-AZURE-9.1",
                                    "check": "Port 22 (SSH) is Open to World",
                                    "severity": "High",
                                    "status": "FAIL",
                                    ...
                                    "resource_id": rule.id,
                                    "region": nsg.location,
                                    "description": f"NSG '{nsg.name}' allows inbound public access (0.0.0.0/0) to SSH (port 22) via rule '{rule.name}'.",
                                    "remediation": "Restrict SSH access to known IP addresses, or use Azure Bastion.",
                                ...
                                findings.append({
                                    ...
                                    "resource_id": rule.id,
                                    "region": nsg.location,
                                    "description": f"NSG '{nsg.name}' does not allow inbound public access (0.0.0.0/0) to SSH (port 22) via rule '{rule.name}'.",
        ...
        print(f"   [!] Error querying Azure subscriptions for NSG SSH check: {e}")
